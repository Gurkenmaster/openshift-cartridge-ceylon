...
import ceylon.demo.net.todo.domain {
	Task
}
...
Table taksList(Collection<Task> tasks, String q) {
				...
				Div {
					children = {
						Input {
							type = checkbox;
							checked = task.done;
							nonstandardAttributes = ["onclick"->"document.location='?q=``q``&`` task.done then "markNotDone" else "markDone" ``=``task.id``'"];
						},
						Span {
				...
				Div {
					children = {
						Span {
							...
							nonstandardAttributes = ["onclick"->("document.location='?q=``q``&remove=``task.id``'")];
						}
					};
				}
			};
	...
	value rows = tasks.collect((task) => Tr {
				column1(task),
				column2(task)
			});
	return Table {
